...
describe('Domain Handler — ADHD Coach', () => {
  ...
  test('has 41 capabilities (Fase 4.3 añade Eisenhower: prioritize_tasks_start, prioritize_step, next_action)', () => {
    ...
    // Fase 3
    expect(caps.find(c => c.name === 'add_reminder')).toBeDefined();
    expect(caps.find(c => c.name === 'list_reminders')).toBeDefined();
    expect(caps.find(c => c.name === 'cancel_reminder')).toBeDefined();
    expect(caps.find(c => c.name === 'complete_reminder_with_time')).toBeDefined();
    expect(caps.find(c => c.name === 'show_overdue_reminders')).toBeDefined();
    // Fase 4 NL
    expect(caps.find(c => c.name === 'explain_commands')).toBeDefined();
    expect(caps.find(c => c.name === 'explain_natural_language')).toBeDefined();
    expect(caps.find(c => c.name === 'what_can_you_do')).toBeDefined();
    // Refactor /agenda (Fase 3): nuevo handler de selección.
    expect(caps.find(c => c.name === 'agenda_confirm_selection')).toBeDefined();
  });
